...
def main():
    ...
    model.load_state_dict(torch.load('model4_upgrade_anime_rating_model.pth', map_location=device))
    ...
    while True:
        ...
        if choice == '1':
            title = input("请输入动漫标题: ")
            summary = input("请输入动漫简介: ")
            prediction = predict_single(model, tokenizer, title, summary, device)
            print(f"\n动漫: {title}")
            print(f"简介: {summary[:100]}...")  # 只打印前100个字符
        ...
        逐渐沉入海中的平静小镇，少年和机器人少女的难忘夏日就这么开始了。"""
            ...
            # 进行预测
            predictions = predict_batch(model, test_dataloader, device)

            # 打印结果
            for i, prediction in enumerate(predictions):
                print(f"\n动漫: {test_data[i]['title']}")
